...
# and_op = lambda x,ax=-1:   tf.reduce_min( x  , axis=ax,name='my_prod') 
# or_op = lambda x,ax=-1:    tf.reduce_max( x  , axis=ax) 

eps=1.e-6

or_op = lambda x,ax=-1 ,keepdims=False: 1.0-tf.math.exp ( tf.math.reduce_sum( tf.math.log( (1.0-x+eps)/(1+eps))  , axis=ax , keepdims=keepdims))  
and_op = lambda x,ax=-1,keepdims=False : tf.math.exp ( tf.math.reduce_sum(  tf.math.log( (x+eps)/(1+eps)  )  , axis=ax,name='my_prod',keepdims=keepdims)) 
