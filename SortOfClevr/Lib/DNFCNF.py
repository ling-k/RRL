...
class DNFCNF(PredFunc):
    ...
    def pred_func(self,xi,xcs=None,t=0):
        ...
        temp = logic_layer_and( xi, self.terms ,  col=self.col, name=self.name+'_AND1', sig=self.sig, mean=self.mean_and,std=self.std_and,w_init=wa,trainable=self.trainable)
        res1 = logic_layer_or( temp, 1 ,  col=self.col, name=self.name+'_OR1',  sig=self.sig, mean=self.mean_or,std=self.std_or,w_init=wo,trainable=self.trainable ) 
        ...
        res2 = logic_layer_and( temp, 1 ,  col=self.col, name=self.name+'_AND2',  sig=self.sig, mean=self.mean_or,std=self.std_or,w_init=wo,trainable=self.trainable ) 
