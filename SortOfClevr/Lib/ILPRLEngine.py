...
class ILPRLEngine(object):
    ...
    def __init__(self,args,predColl,bgs,disp_fn=None ):
        ...
        for p in self.predColl.preds:
            ...
            if p.oname not in self.X0:
                self.X0[p.oname] =          tf.placeholder("float32", [self.args.BS,self.predColl.pairs_len[p.oname]] , name='input_x_' + p.oname)
                if p.pFunc is not None:
                    self.target_data[p.oname] = tf.placeholder("float32", [self.args.BS,self.predColl.pairs_len[p.oname]] , name='target_data_' + p.oname)
                    self.target_mask[p.oname] = tf.placeholder("float32", [self.args.BS,self.predColl.pairs_len[p.oname]] , name='target_mask_' + p.oname)
    ...
    def getTSteps(self,_X0):
        ...
        for t in range(self.args.T):
            ...
            self.xxxx = tf.concat( list( XOs.values() ), -1)
            for p in self.predColl.preds:
                ...
                if p.pFunc is not None and p.pFunc.neg:
                    xi =  and_op( 1.0-xi,-1)
                else:
