...
class DNFCNF2(PredFunc):
    def __init__(self,name='',trainable=True,terms=4,sig=1.0,init=[.1,-1,.1,.1],init_terms=[],post_terms=[],predColl=None,off_w=-10,fast=False,neg=False,middle=100):
    ...
    def pred_func(self,xi,xcs=None,t=0):
        ...
        if not self.fast:
            ...
            if len(self.init_terms)>0:
                ...
                for i,a in enumerate(self.init_terms):
                    ...
                    for item in a.split(', '):
                        # print(item)
                        # print(self.predColl[self.name].get_term_index(item))
        ...
        temp = logic_layer_and( xi, self.terms*10 ,  col=self.col, name=self.name+'_AND1', sig=self.sig, mean=self.mean_and,std=self.std_and,w_init=wa,trainable=self.trainable)
        res1 = logic_layer_or( temp, self.middle ,  col=self.col, name=self.name+'_OR1',  sig=self.sig, mean=self.mean_or,std=self.std_or,w_init=wo,trainable=self.trainable ) 

        # temp = logic_layer_or( xi, self.terms ,  col=self.col, name=self.name+'_OR2', sig=self.sig, mean=self.mean_and,std=self.std_and,w_init=wa,trainable=self.trainable)
        # res2 = logic_layer_and( temp, self.middle ,  col=self.col, name=self.name+'_AND2',  sig=self.sig, mean=self.mean_or,std=self.std_or,w_init=wo,trainable=self.trainable ) 

        # temp = logic_layer_and( tf.concat( (res1,res2),-1), self.terms ,  col=self.col, name=self.name+'_AND3', sig=self.sig, mean=self.mean_and,std=self.std_and,w_init=wa,trainable=self.trainable)
        temp = logic_layer_and(res1 , self.terms ,  col=self.col, name=self.name+'_AND3', sig=self.sig, mean=self.mean_and,std=self.std_and,w_init=wa,trainable=self.trainable)
        res = logic_layer_or( temp, 1 ,  col=self.col, name=self.name+'_OR3',  sig=self.sig, mean=self.mean_or,std=self.std_or,w_init=wo,trainable=self.trainable )
        ...
        for t in self.post_terms:
            ...
            if t[0]=='and':
                if ind>=xi.shape.as_list()[-1]:
                    ind -=xi.shape.as_list()[-1]
                    res = res * (1.0-xi[:,ind:(ind+1)] )
