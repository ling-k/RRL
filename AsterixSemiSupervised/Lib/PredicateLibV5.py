#added feature : if init_terms is provided, restrict the search to those entries
...
class Background:
    ...
    def add_backgroud( self,pred_name , pair ,value=1): 
        if self.predColl[pred_name] is None:
            return                
...
class Predicate:
    ...
    def __init__(self,dname,oname=None, arguments=[],variables=[],pFunc=None, inc_preds=None,exc_preds=None,use_cnt_vars=False, use_neg=False,arg_funcs=None,inc_cnt=None,exc_cnt=None,Fam='eq', exc_terms=[],exc_conds=[],max_T=100,chunk_count=0,pairs=None,count_type=None,count_th=None):
...
class PredCollection:
    ...
    def get_constant_list( self,pred , vl ):
        ...
        for i,cl in enumerate( pred.arguments+pred.variables ):
            # print(i,cl,pred.dname)
            Cs[cl].append( vl[i])
            if cl[0]=='N':
                if pred.use_M: 
                    Cs[cl].append('M_' + vl[i] )
                if pred.use_P: 
                    Cs[cl].append('P_' + vl[i] )
    ...
    def map_fn( self,pair_arg,pair_val ,pred ):
            ...
            for p in self.outpreds:
                ...
                # pts=np.argwhere( pred.exc_term_inds_mask[p.oname] ).flatten()
                for i in pred.exc_term_inds_mask[p.oname] :
                # for i,n in  enumerate(name_set):
                    # if i in pred.exc_term_inds[p.oname]:
                    # if pred.exc_term_inds_mask[p.oname][i]:
                    #     continue
                    
                    try:
                        # ind = p.pairs.index(n)
                        ind = self.rev_pairs_index[p.oname][ name_set[i]]
    ...
    def initialize_predicates(self):
        ...
        for pred in self.preds:
            ...
            for p in self.outpreds:
                ...
                pred.exc_term_inds_mask[p.oname]=[]
                ...
                for i,n in enumerate(name_set):
                    term = p.oname + '(' + ','.join(n)+')'
                    exc=False
                    if pred.pFunc is not None:
                        if pred.pFunc.fast:
                            if not any([ (term in t) for t in pred.pFunc.init_terms]):
                                # print(pred.dname)
                                exc=True
                    ...
                    if term not in pred.exc_terms and not pcond and not exc:
                        Li+=1
                        pred.inp_list.append( term)
                        pred.exc_term_inds_mask[p.oname].append(True)
                    else:
                        pred.exc_term_inds[p.oname].append(i)
                        pred.exc_term_inds_mask[p.oname].append(False)
                     
                pred.Lx_details.append(Li)
                pred.Lx_details_dic[p.oname]=Li
                
                pred.exc_term_inds_mask[p.oname] = np.argwhere( pred.exc_term_inds_mask[p.oname] ).flatten()
